...
let acornSimulator = {
  ...
  simulate: async (ast, scope, thisScope, others)=>{
    await sleep(0);
    if(STOPflag) return {type: "Literal", value: undefined};
    ...
    for(track.line in ast.body){
      ...
      switch(element.type){
        case "FunctionDeclaration":{
          await acornSimulator.declareVariable(element.id,others?.dynamic?_dynamic.properties.any:element,undefined,"function",[],scope);
          break;
        }
        case "VariableDeclaration":{
          let kind = element.kind;
          for(let declaration of element.declarations) {
            await acornSimulator.declareVariable(declaration.id,others?.dynamic?_dynamic.properties.any:declaration.init,undefined,kind,[],scope,undefined,thisScope,undefined);
          };
					break;
        }
        case "ClassDeclaration":{
          await acornSimulator.declareVariable(element.id,others?.dynamic?_dynamic.properties.any:element,undefined,"class",[],scope);
          break;
        }
        case "ImportDeclaration":{
        ...
        case "IfStatement":{
          let test = acornSimulator.coerce(await acornSimulator.resolve(element.test,undefined,[],scope,thisScope,thisScope));
          
          if(test.value === _dynamic.properties.any.value){
            await acornSimulator.simulate({body:[element.consequent]},scope,thisScope,{dynamic:true});
            if(element.alternate) await acornSimulator.simulate({body:[element.alternate]},scope,thisScope,{dynamic:true});
            
            return _dynamic.properties.any;
          }
          let r
          if(test.value){
            r = await acornSimulator.simulate({body:[element.consequent]},scope,thisScope,others);
          }else if(element.alternate){
            r = await acornSimulator.simulate({body:[element.alternate]},scope,thisScope,others);
          }else{
        ...
        case "ForStatement":{
          ...
          if(element.init){
            if(element.init.type === "VariableDeclaration"){
              ...
              await acornSimulator.simulate({body:[element.init]},forInitScope,thisScope,others);
            }else{
              await acornSimulator.resolve(element.init,undefined,[],forInitScope,thisScope,thisScope);
            }
          }
          ...
          const testValue = await acornSimulator.resolve(element.test, undefined, [], scope, thisScope, thisScope).value;
          ...
          if (testValue === externals.find(ext => ext.name === "_Dynamic_").properties.any.value) {
            ...
            switch(element.body.type){
              case "BlockStatement":
                sim = await acornSimulator.simulate(element.body, forScope, thisScope, {dynamic:true});
                break;
              case "ExpressionStatement":
                sim = await acornSimulator.simulate({body:[element.body]}, forScope, thisScope, {dynamic:true});
                break;
              case "EmptyStatement":
          ...
          while(stackCount < stackLimit){
            ...
            if(test.value || test.value===undefined){
              ...
              switch(element.body.type){
                case "BlockStatement":
                  sim = await acornSimulator.simulate(element.body, forScope, thisScope, others);
                  break;
                case "ExpressionStatement":
                  sim = await acornSimulator.simulate({body:[element.body]}, forScope, thisScope, others);
                  break;
                case "EmptyStatement":
                  sim = {type:"Literal",blocked:false,value:undefined};
                  break;
              ...
              if(sim.type==="ReturnSignal") return sim;
							...
							await acornSimulator.resolve(element.update,undefined,[],forScope, thisScope, thisScope);
            }else{
              break;
            }
            switch(element.body.type){
        ...
        case "ForInStatement":{
          let right = await acornSimulator.resolve(element.right,undefined,[],scope,thisScope,thisScope);
          let indexer;
          let i=0;
          let TDZs = [];
          ...
          if(element.left.type === "VariableDeclaration"){
            ...
            await acornSimulator.simulate({body:[element.left]},forLeftScope);
            indexer = acornSimulator.remember(element.left.declarations[0].id.name);
          }else{
            indexer = await acornSimulator.resolve(element.left,undefined,[],forLeftScope,thisScope,thisScope);
          }
        ...
        /*sort of*/case "ForOfStatement":{
          let right = await acornSimulator.resolve(element.right);
          let indexer;
          let i=0;
          let TDZs = [];
        ...
        case "WhileStatement":{
          ...
          let forScope = new Object({name:scope.name+"/forLoopBody"})
          if(await acornSimulator.resolve(element.test, undefined, [], scope, thisScope, thisScope).value === externals.find(ext => ext.name === "_Dynamic_").properties.any.value){
            let sim;
            switch(element.body.type){
              case "BlockStatement":
                sim = await acornSimulator.simulate(element.body, forScope, thisScope, {dynamic:true});
        ...
        case "DoWhileStatement":{
          ...
          while(stackCount < stackLimit){
            ...
            test = acornSimulator.coerce(await acornSimulator.resolve(element.test, undefined, [], scope, thisScope, thisScope));
        ...
        case "SwitchStatement":{
          if(element.discriminant.value === _dynamic.properties.any.value){
            ...
            for(let i = 0; i<element.cases.length;i++){
              await acornSimulator.simulate({body:[...element.cases[i].consequent]},scope,thisScope,{dynamic:true});
            }
          ...
          while(caseID<element.cases.length){
            let test = acornSimulator.coerce(await acornSimulator.resolve({type:"BinaryExpression", operator:"===", left:element.discriminant, right:element.cases[caseID].test},undefined,[],scope,thisScope,thisScope));
            let r
            if(test.value){
              while(caseID<element.cases.length){
                r = await acornSimulator.simulate({body:[...element.cases[caseID].consequent]},scope,thisScope);
        ...
        case "ReturnStatement":{
          let r = await acornSimulator.resolve(element.argument,undefined,[],scope, thisScope,thisScope,others);
          return { type: "ReturnSignal", return: r };
        }
        case "ThrowStatement":{
          let r = await acornSimulator.resolve(element.argument,undefined,[],scope,undefined,thisScope,others);
        ...
        case "TryStatement":{
          ...
          signal = await acornSimulator.simulate(element.block, scope+"/tryBlock", thisScope);
          if(signal?.type === "ErrorSignal" && element.handler) {
            await acornSimulator.declareVariable(element.handler.param,signal.message,undefined,"init",[],scope+"/catch");

            const catchSignal = await acornSimulator.simulate(element.handler.body, scope+"/catchBody", thisScope);
            if(catchSignal) signal = catchSignal;
          }
          
          if(element.finalizer) {
              const finallySignal = await acornSimulator.simulate(element.finalizer, scope+"/finallyBlock", thisScope);
        ...
        case "ExpressionStatement":{
          result = await acornSimulator.resolve(element.expression,undefined,[],scope,thisScope,thisScope,others);
          break;
        }
        case "BlockStatement":{
          let TDZs = acornSimulator.prepare(element.body);
          let result = await acornSimulator.simulate(element,scope+"/block",thisScope,others);
  ...
  declareVariable: async (id,init,def,kind,indexing,scope,selfScope,thisScope,excludedKeys)=>{
    switch(id.type){
      case "Identifier":
        {
          let variable = {
          ...
          let resolution = init.type === "MemberExpression" ? await acornSimulator.resolve(init,def,indexing,scope,selfScope,thisScope).child : await acornSimulator.resolve(init,def,indexing,scope,selfScope,thisScope);
          ...
          switch(resolution.type){
            ...
            case "Function":
              ...
              break;
            case "ReturnSignal":
              await acornSimulator.declareVariable(id,resolution.return,def,kind,indexing,scope,selfScope,thisScope,excludedKeys);
              skip = true;
              break;
            case "ArrowFunction":
              variable.id = resolution.id;
            ...
            case "Closure":
              ...
              break;
            case "Evaluation":
              await acornSimulator.declareVariable(id,resolution.get(),def,kind,indexing,scope,selfScope,thisScope,excludedKeys);
              skip = true;
              break;
            case "Accessor":
              await acornSimulator.declareVariable(id,resolution.get(),def,kind,indexing,scope,selfScope,thisScope,excludedKeys);
      ...
      case "ArrayPattern":
        {
          ...
          for(let index in id.elements){
            ...
            if(indexing){
              await acornSimulator.declareVariable(id.elements[index],init,undefined,kind,[...indexing,id.elements[index].type === "RestElement" ? "a"+index : index],scope,selfScope,thisScope,id.elements[index].type === "RestElement" ? excludedKeys : undefined);
            }else{
              await acornSimulator.declareVariable(id.elements[index],init,undefined,kind,[id.elements[index].type === "RestElement" ? "a"+index : index],scope,selfScope,thisScope);
            }
          }
          break;
        }
          ...
          for(let index in id.properties){
            ...
            if(indexing){
              if(id.properties[index].type === "Property"){
                ...
                await acornSimulator.declareVariable(id.properties[index].value,init,undefined,kind,[...indexing,id.properties[index].key.name],scope,selfScope,thisScope);
              }else if(id.properties[index].type === "RestElement"){
                await acornSimulator.declareVariable(id.properties[index],init,undefined,kind,[...indexing,undefined],scope,selfScope,thisScope,excludedKeys);
              }
            }else{
              //switch/case if it hit a third.
              if(id.properties[index].type === "Property"){
                excludedKeys.push(id.properties[index].key.name);
                await acornSimulator.declareVariable(id.properties[index].value,init,undefined,kind,[id.properties[index].key.name],scope,selfScope,thisScope);
              }else if(id.properties[index].type === "RestElement"){
                await acornSimulator.declareVariable(id.properties[index],init,undefined,kind,[undefined],scope,selfScope,thisScope,excludedKeys);
              }
            }
          }
          break;
      ...
      case "AssignmentPattern":
        {
          await acornSimulator.declareVariable(id.left,init,id.right,kind,[...indexing],scope,selfScope,thisScope);
          break;
        }
      case "RestElement":
        {
          ...
          if(init.type === "ArrayExpression"){
            ...
            if(indexing&&indexing.length>0&&indexing[0][0] !== "a"[0]){
              ...
              await acornSimulator.declareVariable(id,init.elements[indexing[0]],def,kind,newIndexing,scope,selfScope,thisScope,excludedKeys);
              break;
            }
            for(let i=indexing[0].split("a")[1];i<init.elements.length;i++){
              elements.push(await acornSimulator.resolve(init.elements[i],def,[],scope,selfScope,thisScope))
            }
            variable.type = "Array";
          ...
          if(init.type === "ObjectExpression"){
            ...
            if(indexing&&indexing.length>0&&indexing[0] !== undefined){
              let newIndexing = [...indexing];
              newIndexing.splice(0,1);
              await acornSimulator.declareVariable(id,init.properties.find(prop => prop.key.name === indexing[0]).value,def,kind,newIndexing,scope,selfScope,thisScope,excludedKeys);
              break;
            }
            for(let prop of init.properties){
              if(!excludedKeys.includes(prop.key.name)){
                let resolution = await acornSimulator.resolve(prop.value,def,indexing,scope,selfScope,thisScope);
  ...
  assignVariable: async (id,init,def,indexing,scope,selfScope,thisScope,excludedKeys)=>{
    switch(id.type){
      case "Identifier":
        {
          ...
          if(tarjet.kind !== "const"){
            ...
            let resolution = await acornSimulator.resolve(init,def,indexing,scope,selfScope,thisScope);
            ...
            switch(resolution.type){
              ...
              case "Function":
                ...
                break;
              case "ReturnSignal":
                await acornSimulator.resolveVariable(id,resolution.return,def,indexing,scope,selfScope,thisScope,excludedKeys);
                skip = true;
                break;
              case "ArrowFunction":
                tarjet.value = undefined;
      ...
      case "ArrayPattern":
        {
          ...
          for(let index in id.elements){
            let element = id.elements[index].value;
            if(indexing){
              await acornSimulator.assignVariable(id.elements[index],init,undefined,[...indexing,id.elements[index].type === "RestElement" ? "a"+index : index],scope,selfScope,thisScope,id.elements[index].type === "RestElement" ? excludedKeys : undefined);
            }else{
              await acornSimulator.assignVariable(id.elements[index],init,undefined,[id.elements[index].type === "RestElement" ? "a"+index : index],scope,selfScope,thisScope);
            }
          }
          break;
        }
      case "ObjectPattern":
        {
          ...
          for(let index in id.properties){
            ...
            if(indexing){
              if(id.properties[index].type === "Property"){
                excludedKeys.push(id.properties[index].key.name);
                await acornSimulator.assignVariable(id.properties[index].value,init,undefined,[...indexing,id.properties[index].key.name],scope,selfScope,thisScope);
              }else if(id.properties[index].type === "RestElement"){
                await acornSimulator.assignVariable(id.properties[index],init,undefined,[...indexing,undefined],scope,selfScope,thisScope,excludedKeys);
              }
            }else{
              //switch/case if it hit a third.
              if(id.properties[index].type === "Property"){
                excludedKeys.push(id.properties[index].key.name);
                await acornSimulator.assignVariable(id.properties[index].value,init,undefined,[id.properties[index].key.name],scope,selfScope,thisScope);
              }else if(id.properties[index].type === "RestElement"){
                await acornSimulator.assignVariable(id.properties[index],init,undefined,[undefined],scope,selfScope,thisScope,excludedKeys);
              }
            }
          }
          break;
      ...
      case "AssignmentPattern":
        {
          await acornSimulator.assignVariable(id.left,init,id.right,[...indexing],scope,selfScope,thisScope);
          break;
        }
      case "RestElement":
        {
          ...
          if(init.type === "ArrayExpression"){
            ...
            if(indexing&&indexing.length>0&&indexing[0][0] !== "a"[0]){
              let newIndexing = [...indexing];
              newIndexing.splice(0,1);
              await acornSimulator.assignVariable(id,init.elements[indexing[0]],def,newIndexing,scope,selfScope,thisScope,excludedKeys);
              break;
            }
            for(let i=indexing[0].split("a")[1];i<init.elements.length;i++){
              elements.push(await acornSimulator.resolve(init.elements[i],def,[],scope,selfScope,thisScope))
          ...
          if(init.type === "ObjectExpression"){
            ...
            if(indexing&&indexing.length>0&&indexing[0] !== undefined){
              let newIndexing = [...indexing];
              newIndexing.splice(0,1);
              await acornSimulator.assignVariable(id,init.properties.find(prop => prop.key.name === indexing[0]).value,def,newIndexing,scope,selfScope,thisScope,excludedKeys);
              break;
            }
            for(let prop of init.properties){
              if(!excludedKeys.includes(prop.key.name)){
      ...
      case "MemberExpression":
        {
          let kinda = await acornSimulator.resolve(id,def,indexing,scope,selfScope,thisScope);
          let property = id.computed ? await acornSimulator.resolve(id.property,def,indexing,scope,selfScope,thisScope).value : id.property.name
          let object;
          if(kinda.child?.value === undefined){
            object = kinda.parent.type === "Identifier" ? await acornSimulator.resolve(kinda.parent,undefined,[],scope,selfScope,thisScope) : kinda.parent.type === "MemberExpression" ? await acornSimulator.resolve(kinda.parent,undefined,[],scope,selfScope,thisScope).child : kinda.parent;
            tarjet = object.properties?.[property];
          }else{
            object = await acornSimulator.resolve(kinda.parent,undefined,[],scope,selfScope,thisScope);
            tarjet = object.properties?.[property];
          }
          ...
          let resolution = await acornSimulator.resolve(init,def,indexing,scope,object,thisScope);
          resolution = resolution.child?resolution.child:resolution;
          ...
          switch(resolution.type){
            ...
            case "Function":
              ...
              break;
            case "ReturnSignal":
              await acornSimulator.assignVariable(id,resolution.return,def,indexing,scope,object,thisScope,excludedKeys);
              skip = true;
              break;
            case "ArrowFunction":
              tarjet.value = undefined;
  ...
  resolve: async (ast,def,indexing,scope,selfScope,thisScope,others)=>{
    await sleep(0);
    if(STOPflag) return {type: "Literal", value: undefined};
    const _dynamic = externals.find(ext => ext.name === "_Dynamic_");
    if(others?.dynamic){
      ...
      switch(ast.type){
        ...
        case "TemplateLiteral":{
          ...
          for(let q in ast.quasis){
            ...
            if(!quack.tail) value += await acornSimulator.resolve(ast.expressions[q],def,indexing,scope,selfScope,thisScope).value;
          }
        ...
        case "ArrayExpression":{
          if(indexing&&indexing.length>0){
            ...
            return await acornSimulator.resolve(r ? r : def, def ,newIndexing,scope, selfScope, thisScope);
          }else{
            let elements = [];
            for(let element of ast.elements){
              elements.push(await acornSimulator.resolve(element,undefined,[],scope, selfScope, thisScope));
            }
            ...
            Object.defineProperty(toreturn.properties, "length", {
              get() {
                return {
                  type: "Literal",
                  blocked: false,
                  value: toreturn.elements.length
                };
              },
              set(newVal) {
                // Optionally: handle array truncation logic if you want to mirror JS behavior
                if (typeof newVal.value === "number") {
                  toreturn.elements.length = newVal.value;
                }
              },
              enumerable: true,
              configurable: true
        ...
        case "ObjectExpression":{
          if(indexing&&indexing.length>0){
            ...
            return await acornSimulator.resolve(r ? r.value : def, def ,newIndexing,scope, selfScope, thisScope);
          }else{
            let properties = {};
            for(let property of ast.properties){
              let key=property.computed? acornSimulator.coerce(await acornSimulator.resolve(property.key,undefined,[],scope, selfScope, thisScope)).value : property.key.name;
              properties[key]=await acornSimulator.resolve(property.value,def,[],scope, selfScope, thisScope);
            }
        ...
        case "ClassDeclaration":{
          return {type:"Class", blocked: false, prototype:new Object(), properties:{}, id:ast.id, super:await acornSimulator.resolve(ast.superClass,def,[],scope,selfScope,thisScope), body:ast.body.body};
        }
        /**/case "MetaProperty":{
        ...
        case "UnaryExpression":{
          switch(ast.operator){
            case "+":
              {
                let resolution = ast.argument.type === "MemberExpression" ? await acornSimulator.resolve(ast.argument,undefined,[],scope,selfScope,thisScope).child : await acornSimulator.resolve(ast.argument,undefined,[],scope,selfScope,thisScope);
                  ...
                  case "Literal":
                    if(resolution.value===_dynamic.properties.any.value){
                      ...
                      for(let val of resolution.value.possibilities){
                        ...
                        values.push(await acornSimulator.resolve({type:"UnaryExpression",operator:"typeof",argument:va},def,indexing,scope,selfScope,thisScope));
                      }
                      return _dynamic.properties.possibilities.run(...values);
                    }
            ...
            case "void":
              {
                await acornSimulator.resolve(ast.argument,undefined,[],scope,selfScope,thisScope);
                return {type: "Literal", blocked: false, value: undefined};
              }
            case "delete":
              {
                if(ast.argument.type === "MemberExpression"){
                  let object = ast.argument.object.type === "MemberExpression" ? ast.argument.type === "MemberExpression" ? await acornSimulator.resolve(ast.argument.object,undefined,[],scope,selfScope,thisScope).child : await acornSimulator.resolve(ast.argument.object,undefined,[],scope,selfScope,thisScope) : acornSimulator.remember(ast.argument.object.name);
                  if(object===_dynamic){
                    console.warn(`Attempt to delete dynamicly`);
                    acornSimulator.safe = false;
                    return { type: "Literal", blocked: false, value: true };
                  };
                  let propertyName = ast.argument.computed ? ast.argument.type === "MemberExpression" ? await acornSimulator.resolve(ast.argument.property,undefined,[],scope,selfScope,thisScope).child.value : await acornSimulator.resolve(ast.argument.property,undefined,[],scope,selfScope,thisScope).value : ast.argument.property.name;
                  if(object){
                    switch(object.type){
                      case "Object":
        ...
        case "UpdateExpression":{
          let variable = ast.argument.type === "Identifier" ? acornSimulator.remember(ast.argument.name) : ast.argument.type === "MemberExpression" ? await acornSimulator.resolve(ast.argument,undefined,[],scope,selfScope,thisScope).child : await acornSimulator.resolve(ast.argument,undefined,[],scope,selfScope,thisScope);
          ...
          if(coertion.value===_dynamic.properties.any.value){
            ...
            await acornSimulator.assignVariable(ast.argument,{type: "Literal", blocked: false,value: newValue},undefined,[],scope,selfScope,thisScope);
          }
        ...
        case "BinaryExpression":{
          let left = ast.left.type === "MemberExpression" ? await acornSimulator.resolve(ast.left,undefined,[],scope,selfScope,thisScope).child : await acornSimulator.resolve(ast.left,undefined,[],scope,selfScope,thisScope);
          let leftCoertion = acornSimulator.coerce(left);
          let right = ast.right.type === "MemberExpression" ? await acornSimulator.resolve(ast.right,undefined,[],scope,selfScope,thisScope).child : await acornSimulator.resolve(ast.right,undefined,[],scope,selfScope,thisScope);
        ...
        case "LogicalExpression":{
          let left = await acornSimulator.resolve(ast.left,undefined,[],scope,selfScope,thisScope);
          ...
          if(coertion.value===_dynamic.properties.any.value){
            ...
            for(let pos of coertion.value.possibilities){
              toreturn.possibilities.push(await acornSimulator.resolve({type:"LogicalExpression",left:pos,right:ast.right,operator:ast.operator},def,indexing,scope,selfScope,thisScope))
            }
            return {
              type:"Literal",
              blocked: false,
          ...
          switch(ast.operator) {
            case "&&":{
              ...
              let resolution = await acornSimulator.resolve(ast.right,undefined,[],scope,selfScope,thisScope);
        ...
        case "AssignmentExpression":{
          switch(ast.operator){
            case "=":
              {
                let assignation = await acornSimulator.assignVariable(ast.left,ast.right,def,indexing,scope,selfScope,thisScope);
                
                let toreturn = {
                  type: assignation ? assignation.type : "Literal",
                }
                switch(toreturn.type){
                  case "Literal":
                    if(assignation.value){
                      toreturn.value = assignation.value;
                    }else if(def){
                      toreturn.value = def.value;
                    }else{
                      toreturn.value = undefined;
                    }
                    break;
                  case "Array":
                    toreturn.elements = assignation.elements;
                    toreturn.properties = assignation.properties;
                    break;
                  case "Object":
                    toreturn.properties = assignation.properties;
                    break;
                  case "Function":
                    toreturn.id = assignation.id;
                    toreturn.params = assignation.params;
                    toreturn.body = assignation.body;
                    break;
                  case "ArrowFunction":
                    toreturn.id = assignation.id;
                    toreturn.params = assignation.params;
                  ...
                  case "Class":
                    toreturn.id = assignation.id;
                    toreturn.super = assignation.super;
                    toreturn.body = assignation.body;
                    break;
                  case "Closure":
                    toreturn.value = assignation.value;
                    toreturn.env = assignation.env;
                    break;
                  default:
                    console.warn(`Unhandled '${toreturn.type}' assignation return`);
                    acornSimulator.safe = false;
                    break;
                }
                return toreturn;
            ...
            case "+=":
              {
                let res = await acornSimulator.resolve(ast.left);
                let tarjet = res.child ? res.child : res;//acornSimulator.remember(ast.left.name);
                
                let resolution = await acornSimulator.resolve(ast.right,def,indexing,scope,selfScope,thisScope);
                resolution = resolution.child?resolution.child:resolution;
                ...
                await acornSimulator.assignVariable(ast.left,tarjet,def,indexing,scope,selfScope,thisScope);
        ...
        case "ConditionalExpression": {
          let test = await acornSimulator.resolve(ast.test, undefined, [], scope, selfScope, thisScope);
          ...
          if(test.value===_dynamic.properties.any.value){
            ...
            for(let pos of test.value.possibilities){
              possibilities.push(acornSimulator.coerce(pos).value? await acornSimulator.resolve(ast.consequent, undefined, [], scope, selfScope, thisScope)
          : await acornSimulator.resolve(ast.alternate, undefined, [], scope, selfScope, thisScope));
            }
          ...
          return testValue
            ? await acornSimulator.resolve(ast.consequent, undefined, [], scope, selfScope, thisScope)
          : await acornSimulator.resolve(ast.alternate, undefined, [], scope, selfScope, thisScope);
        }
        case "SequenceExpression":{
          let result = {type: "Literal", value: undefined};
          for(let expression of ast.expressions){
            result = await acornSimulator.resolve(expression,undefined,[],scope,selfScope,thisScope);
          }
          result = result.child?result.child:result;
          return result;
        }
        case "CallExpression":{
          let resolution = await acornSimulator.resolve(ast.callee,undefined,[],scope,undefined,thisScope);
          ...
          if(ast.callee.type === "MemberExpression"){
            ...
            if(resolution.child.type === "Closure"){
              ...
              let result = await acornSimulator.call(resolution.child.value,ast.callee,ast.arguments,scope,thisScope);
              ...
              return await acornSimulator.call(resolution.child,ast.callee,ast.arguments,scope,thisScope);
            }
            ...
            let result = await acornSimulator.call(resolution.value,ast.callee,ast.arguments,scope,thisScope);
            for(let f of Forgetables){
              f.TDZ = true;
            }
            for(let t of TDZs){
            ...
            return await acornSimulator.call(resolution,ast.callee,ast.arguments,scope,thisScope);
          }
        }
        case "NewExpression":{
          let callee = await acornSimulator.resolve(ast.callee);
          switch(callee.type){
            case "Class":{
              ...
              for(let line of callee.body){
                ...
                internals[line.computed? acornSimulator.coerce(await acornSimulator.resolve(line.key,undefined,[],scope,selfScope,thisScope)).value : line.key.name] = await acornSimulator.resolve(line.value);
                if(line.type === "MethodDefinition"){
                  if(line.kind === "constructor") constructor = await acornSimulator.resolve(line.value);
                }
              }
              ...
              if(constructor){
                await acornSimulator.call(constructor,ast.callee,ast.arguments,scope,result);
              }
            ...
            case "Evaluation":
              ...
              for(let arg of ast.arguments){
                args.push(await acornSimulator.resolve(arg,undefined,indexing,scope,selfScope,thisScope));
              }
        ...
        case "MemberExpression":{
          //console.log(ast.object.type === "MemberExpression" ? await acornSimulator.resolve(ast.object,undefined,[],scope,selfScope,thisScope) : "");
          ...
          let resolution = await acornSimulator.resolve(ast.object,undefined,[],scope,selfScope,thisScope)
          ...
          switch(object.type){
            case "Object": {
              let prop = ast.computed ? acornSimulator.coerce(await acornSimulator.resolve(ast.property,undefined,[],scope,ast.object,thisScope)).value : ast.property.name;
  ...
  call: async (resolution,callee,argus,scope,thisScope)=>{
    ...
    if(resolution.type === "Evaluation") {
      ...
      for(let arg of argus){
        let res = await acornSimulator.resolve(arg,undefined,[],scope);
    ...
    for(let i in resolution.params){
      ...
      switch(param.type){
        case "Identifier":
          ...
          await acornSimulator.declareVariable(param,arg,undefined,"init",[],scope+"/function");
          break;
        case "ArrayPattern":
          await acornSimulator.declareVariable(param, arg, undefined, "init", [], scope + "/function");
          break;
        case "ObjectPattern":
          ...
          break;
        case "AssignmentPattern":
          await acornSimulator.declareVariable(param.left, arg !== undefined ? arg : param.right, undefined, "init", [], scope + "/function");
          break;
        case "RestElement":
          await acornSimulator.declareVariable(param, arg, undefined, "init", [], scope + "/function");
          break;
    ...
    let result = await acornSimulator.simulate(resolution,functionScope,resolution.type==="Function" ? mem.type === "Class"? thisScope : callee.type === "MemberExpression"? await acornSimulator.resolve(callee).parent : undefined : resolution.type === "ArrowFunction" ? thisScope : undefined);
...
async function acornScanner(userCode){
	/*
  ...
  try{
    ...
    await sleep(0);
    if(STOPflag) return {type: "Literal", value: undefined};
    await acornSimulator.simulate(pcode,mainScope,undefined);
    
    externals = extPool2;
    await sleep(0);
    if(STOPflag) return {type: "Literal", value: undefined};
    await acornSimulator.simulate(acorn.parse(`
      preload();
      setup();
    ...
    await acornSimulator.simulate(acorn.parse(`draw;`, { ecmaVersion: 'latest', sourceType: 'module', locations: true }), mainScope, undefined, {dynamic:true});
  }catch(err){
    dropError("",err);
    acornSimulator.safe = false;
  }
...
/*self.onmessage = async (event) => {
  ...
  // ✅ Ignore anything not from codeReader
  //if (data?.source !== "codeReader") return;

  if (data.cmd === "Scan") {
    scannerP5Dummy.setup();
    STOPflag = false;
    const result = await acornScanner(data.code);
    self.postMessage({ source: "scannerWorker", cmd: "ScanResult", result });
  } 
