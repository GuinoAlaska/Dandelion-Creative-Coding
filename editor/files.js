...
const editorChangeFunction = (instance) => {
    ...
    if (window.isMobile) {
        ...
        document.getElementById('mobile-project-status').style.color = 'var(--projectNotSaved-color)';
    } else {
        isDirty = true;
        document.getElementById('project-status').innerHTML = "Project not saved";
        document.getElementById('project-status').style.color = 'var(--projectNotSaved-color)';
    }
    if(config.autoSave&&!loading){
        isDirty = false;
        if (window.isMobile) {
