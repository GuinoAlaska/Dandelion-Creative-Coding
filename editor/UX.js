...
function IDEStopSketch(){
    ifr.contentWindow.postMessage({ cmd: 'stop' }, "*");
    ...
    topSketch = (p) => {
        ...
        p.draw = () => {
            p.background(theme? theme==="dark"?0:220:220);
            p.noStroke();
            p.fill(theme? theme==="dark"?255:0:0);
...
function IDEPauseSketch(){
    ifr.contentWindow.postMessage({ cmd: 'pause', data: CodePaused}, "*");
    if (CodePaused) {
        //topP5Instance.loop();
        CodePaused = false;
        document.getElementById('pause-btn').style.backgroundPosition = '-158px -108px';
...
document.querySelector('#tabs-panel .add-tab').addEventListener('click', function () {
    const addTabContent = document.querySelector('.add-tab-content');
    if (addTabContent) {
        addTabContent.style.display = 'flex';
    }
});
...
if (addTabContent) {
    ...
    addTabContent.addEventListener('click', (e) => e.stopPropagation());
    addTabContent.addEventListener('touchstart', (e) => e.stopPropagation());

    const hideAddTabContent = () => {
        addTabContent.style.display = 'none';
    };

    // Hide when clicking/touching outside the panel or the add-tab button
    document.addEventListener('click', (e) => {
        if (!e.target.closest('.add-tab-content') && !e.target.closest('.add-tab')) {
            hideAddTabContent();
        }
    });
...
// new js
document.getElementById('add-js').addEventListener('click', (event) => {
    ...
    tabIcon.style.minWidth = '40px';
    tabIcon.style.height = '40px';
    tabIcon.style.backgroundImage = "var(--TabIcons)"; // sprite sheet
    tabIcon.style.backgroundSize = 'auto'; // or 'contain' if scaling
    tabIcon.style.backgroundRepeat = 'no-repeat';
    tabIcon.style.backgroundPosition = '0 0'; // show first icon
    tabIcon.style.overflow = 'hidden';
    ...
    document.querySelector('.add-tab-content').style.display = 'none';
});

// new txt
document.getElementById('add-txt').addEventListener('click', (event) => {
    // Create a new file object
    newFile("txt");
    ...
    tabIcon.style.backgroundSize = 'auto'; // or 'contain' if scaling
    tabIcon.style.backgroundRepeat = 'no-repeat';
    tabIcon.style.backgroundPosition = '-80px 0'; // show first icon
    tabIcon.style.overflow = 'hidden';
    tab.appendChild(tabIcon);
...
// new json
document.getElementById('add-json').addEventListener('click', (event) => {
    // Create a new file object
    newFile("json");
    ...
    tabIcon.style.backgroundSize = 'auto'; // or 'contain' if scaling
    tabIcon.style.backgroundRepeat = 'no-repeat';
    tabIcon.style.backgroundPosition = '-40px 0'; // show first icon
    tabIcon.style.overflow = 'hidden';
    tab.appendChild(tabIcon);
...
// import
document.getElementById('add-import').addEventListener('click', (event) => {
    fileInput.click();
});
...
function processFile(file){
    ...
    // Define what happens after reading
    reader.onload = (e) => {
        ...
        // Now you can parse/process based on type
        switch(file.type) {
            case "text/javascript":{
                const editor = CodeMirror.fromTextArea(textarea, {
                    mode: 'javascript',
                    theme: theme? theme==="dark"?'dracula':'spotlight':'spotlight',
                    lineNumbers: true,
                    lineWrapping: true,
                    foldGutter: true,
            ...
            default:
                console.log("Unhandled type, you could use ArrayBuffer or DataURL");
                break;
        }
        ...
        switch(file.type) {
            case "text/javascript":{
                tabIcon.style.backgroundPosition = '0 0';
                break;
            }
            case "text/plain":{
                tabIcon.style.backgroundPosition = '-80px 0';
                break;
            }
            case "application/json":{
                tabIcon.style.backgroundPosition = '-40px 0';
                break;
            }
        }
        tabIcon.style.overflow = 'hidden';
    ...
    if (file.type.startsWith("text/") || file.type === "application/json") {
        reader.readAsText(file); // For text and JSON
    } else if (file.type.startsWith("image/") || file.type.startsWith("audio/") || file.type.startsWith("video/")) {
        reader.readAsDataURL(file); // For media playback or display
    } else {
        reader.readAsArrayBuffer(file); // For 3D models, binary files, etc.
    }

    //switchToFile(files.length);
...
document.getElementById('console-input').addEventListener('keydown', (event) => {
    if (event.key === 'Enter') {
        ...
        ifr.contentWindow.postMessage({ cmd: 'evaluate', code: input }, "*");
    ...
    // Navigate console history with Up/Down arrows
    if (event.key === 'ArrowUp' || event.key === 'ArrowDown') {
        ...
        if (event.key === 'ArrowUp') {
            //console.log(consoleHistory[consoleHistoryIndex]);
            if (consoleHistoryIndex > 0) {
                consoleHistoryIndex--;
                event.target.value = consoleHistory[consoleHistoryIndex] || '';
            }
        } else {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById("toggle-tabs").addEventListener("click", () => {
        const isHidden = tabsPanel.classList.toggle("hidden");
        ...
        config.visibility.tabs = !isHidden;
        saveConfig()
    });

    document.getElementById("toggle-console").addEventListener("click", () => {
        const isHidden = consolePanel.classList.toggle("hidden");
        ConsoleResizer.classList.toggle("hidden");
        config.visibility.console = !isHidden;
        saveConfig()
    });
    ...
    document.getElementById("toggle-editorView").addEventListener("click", () => {
        ...
        config.visibility.preview = !isHidden;
        saveConfig()
    });
    ...
    document.getElementById("toggle-sketch").addEventListener("click", () => {
        ...
        config.visibility.sketch = !isHidden;
        saveConfig()
    })
});
...
window.addEventListener("load", () => {
    ...
    const topSketchPanel = document.getElementById("output-top");
    const rightHeight = document.getElementById("right-panel").offsetHeight;

    const topHidden = topSketchPanel.classList.contains("hidden");
    const bottomHidden = document.getElementById("output-bottom").classList.contains("hidden");
    ...
    if(topHidden){
        if(bottomHidden){
            leftPanel.style.width = containerWidth;
            MainResizer.style.left = containerWidth / 2 + "px";
        }else{
            topSketchPanel.style.height = "0px";
            OutputsResizer.style.top = rightHeight / 2 + "px";
        }
    }else{
        if(bottomHidden){
            topSketchPanel.style.height = rightHeight;
            OutputsResizer.style.top = rightHeight / 2 + "px";
        }else{
            topSketchPanel.style.height = rightHeight / 2 + "px";
            OutputsResizer.style.top = rightHeight / 2 + "px";
        }
    }
...
function saveProject() {
    ...
    if (files.length === 0) {
        console.warn("saveProject() aborted: files array is empty");
        return; // 🚫 DO NOT SAVE
    }
    ...
    if(config.autoSave){
        if (window.isMobile) {
            document.getElementById('mobile-project-status').innerHTML = "Autosave Enabled";
            document.getElementById('mobile-project-status').style.color = '#00ffff';
        } else {
            document.getElementById('project-status').innerHTML = "Autosave Enabled";
            document.getElementById('project-status').style.color = '#00ffff';
        }
    }else{
        if (window.isMobile) {
            document.getElementById('mobile-project-status').innerHTML = "Project saved";
            document.getElementById('mobile-project-status').style.color = 'var(--projectSaved-color)';
        } else {
            document.getElementById('project-status').innerHTML = "Project saved";
            document.getElementById('project-status').style.color = 'var(--projectSaved-color)';
        }
    }
}
...
function loadProject() {
    ...
    try {
        ...
        files.forEach((file, idx) => {
            ...
            tab.style.setProperty('--tab-active-border-color', 'var(--search-btn-hover-bg-color)');
            tabsList.appendChild(tab);
    ...
    //scannerWorker.postMessage({ source:"codeReader", cmd:"Stop"});
    //console.log("Send scan");
    //scannerWorker.postMessage({ source:"codeReader", cmd:"Scan", code: gatheringCode });
...
function loadProjectData(project) {
    if (project.files && Array.isArray(project.files)) {
        for(let f of project.files) {
            ...
            const editor = CodeMirror.fromTextArea(textarea, {
                mode: 'javascript',
                theme: editor.theme? editor.theme==="dark"?'dracula':'spotlight':'spotlight',
                lineNumbers: true,
                lineWrapping: true,
                foldGutter: true,
                gutters: ["CodeMirror-linenumbers", "CodeMirror-foldgutter"],
    ...
    files.forEach((file, idx) => {
        const tab = document.createElement('li');
        tab.className = 'tab';
        tab.style.backgroundImage = "var(--TabIcons)";
